...
class MapMaker:
  ...
  def process_scan(self, msg):
    ...
    # print "process_scan > msg"
    # print msg

    number = (msg.angle_max - msg.angle_min)/msg.angle_increment
    # print "Number:", number
    ...
    if self.process_odom_flag ==1:
      for range_value in msg.ranges:
        ...
        # Range Value is a distance from start of laser point (which is robot frame origin)
        # Convert this to coordinate frame of robot and then into world coordinate system

        angle_to_robot_frame = msg.angle_min + ( i * msg.angle_increment )

        # x_ray_point_to_robot_frame = range_value * math.cos(angle_to_robot_frame)

        # if angle_to_robot_frame < 0:
        #   y_ray_point_to_robot_frame = - math.sin(angle_to_robot_frame) * range_value
        # else:
        #   y_ray_point_to_robot_frame = math.sin(angle_to_robot_frame) * range_value

        # # Now convert the point to world coordinate system

        # x_world = ( (math.cos(self.theta_now) * x_ray_point_to_robot_frame) - (math.sin(self.theta_now) * y_ray_point_to_robot_frame) ) + self.x_now
        # y_world = ( (math.sin(self.theta_now) * x_ray_point_to_robot_frame) + (math.cos(self.theta_now) * y_ray_point_to_robot_frame) ) + self.y_now
        ...
        if grid_go_ahead == 1:
          ...
          for j in range(len(points_on_ray)):
            ...
            # else:
            #   self.grid.data[point_on_ray_index] = -1

            # if range_value < msg.range_max:

            #   if range_value > msg.range_min:
